...
// Unified sniper surface. Mounted at /sniper (production URL on the
// mobile bottom nav) and at /mobile-v3-preview/sniper (lab path).
// AppShell + MobileTopHeader so the surface shares the exact same
// shell, bottom nav and lifecycle as the rest of the v3 product.
//
// Pure render-only. Mutations only fire on explicit user clicks. The
// page never marks SEEN, never dispatches alerts, never invents a
// verdict or score.
...
export function SniperSurface() {
  ...
  return (
    <AppShell>
      <MobileTopHeader
        title="Sniper"
        subtitle="Actifs à surveiller sans bruit inutile"
        contextLine={
          focus.length > 0
