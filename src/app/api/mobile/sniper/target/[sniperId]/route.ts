// Pass-through to nx.fn_sniper_delete_target.
...
export async function DELETE(
  req: NextRequest,
  { params }: { params: Promise<{ sniperId: string }> },
) {
  try {
    ...
    const { data, error } = await supabase.rpc("fn_sniper_delete_target", {
      p_sniper_id: sniperId,
      p_user_id: userId,
    });

    if (error) {
      console.error("[/api/mobile/sniper/target/[sniperId] DELETE] rpc error:", error);
      return NextResponse.json(
    ...
    console.error("[/api/mobile/sniper/target/[sniperId] DELETE] error:", err);
    return NextResponse.json(
