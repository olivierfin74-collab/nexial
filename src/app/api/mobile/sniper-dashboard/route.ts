// Pass-through to nx.fn_sniper_dashboard.
...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabase.rpc("fn_sniper_dashboard", {
      p_user_id: userId,
    });

    if (error) {
      console.error("[/api/mobile/sniper-dashboard] rpc error:", error);
      return NextResponse.json(
    ...
    return NextResponse.json({ data: (data as SniperDashboardPayload | null) ?? null });
  } catch (err) {
    const detail = err instanceof Error ? err.message : "Internal error";
    console.error("[/api/mobile/sniper-dashboard] error:", err);
    return NextResponse.json(
