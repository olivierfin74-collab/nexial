// Pass-through to nx.fn_todo_list.

import { NextRequest, NextResponse } from "next/server";
import { createClient } from "@supabase/supabase-js";
import type { TodoListPayload } from "@/types/nexial-v3";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabase.rpc("fn_todo_list", {
      p_user_id: userId,
    });

    if (error) {
      console.error("[/api/mobile/todo-list] rpc error:", error);
      return NextResponse.json(
    ...
    return NextResponse.json({ data: (data as TodoListPayload | null) ?? null });
  } catch (err) {
    const detail = err instanceof Error ? err.message : "Internal error";
    console.error("[/api/mobile/todo-list] error:", err);
    return NextResponse.json(
